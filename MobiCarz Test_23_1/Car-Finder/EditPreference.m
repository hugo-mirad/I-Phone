...
- (void)viewDidLoad
{
    ...
    //
    [self.view addSubview:self.editPrefScrollView];
    //autolayout
    [self.editPrefScrollView setTranslatesAutoresizingMaskIntoConstraints:NO];
    NSLayoutConstraint *scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.editPrefScrollView attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeLeading multiplier:1 constant:0];
    [self.view addConstraint:scrollView1Constraint];
    
    scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.editPrefScrollView attribute:NSLayoutAttributeTrailing relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeTrailing multiplier:1 constant:0];
    [self.view addConstraint:scrollView1Constraint];
    
    
     if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_6_1){
    
    scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.editPrefScrollView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeTop multiplier:1 constant:0];
    [self.view addConstraint:scrollView1Constraint];
    
    scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.editPrefScrollView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeBottom multiplier:1 constant:0];
    [self.view addConstraint:scrollView1Constraint];
     ...
     else
     {
         ...
         scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.editPrefScrollView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeTop multiplier:1 constant:self.navigationController.navigationBar.frame.size.height+rect.size.height];
         [self.view addConstraint:scrollView1Constraint];
         
         scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.editPrefScrollView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeBottom multiplier:1 constant:-tabBarSize.height];
         [self.view addConstraint:scrollView1Constraint];
    ...
    makeLabel.textColor = [UIColor blackColor];
    ...
    self.saveButton.backgroundColor = [UIColor colorWithRed:216.0f/255.0f green:74.0f/255.0f blue:73.0f/255.0f alpha:1.0f];

    [self.saveButton setTitle:@"SAVE" forState:UIControlStateNormal];
    [self.saveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    //Button with 0 border so it's shape like image shape
    self.saveButton.layer.shadowRadius = 1.0f;
...
-(void)makesOperationDownloadErrorNotifMethod:(NSNotification *)notif
{
    ...
    else
    {
        ...
        {
            alert.title=@"Server Error";
            alert.message=@"MobiCarz cannot retrieve data because of server error.";
        }
        [alert show];
        alert=nil;
    }
