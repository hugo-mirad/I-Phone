...
- (void)uploadPhoto
{
    ...
    parameters = [NSDictionary dictionaryWithObjectsAndKeys:carid, @"CarID",makeName, @"make",modelName,@"model",year,@"year",uid,@"UserID",byteArray,@"picContent",@"ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654",@"AuthenticationID",retrieveduuid,@"CustomerID",sessionID,@"SessionID",brandID,@"BrandID",nil];
    
    __weak CameraViewController *weakSelf=self;
    
    [self.Client setParameterEncoding:AFJSONParameterEncoding];
    [self.Client postPath:@"MobileService/GenericCarService.asmx/GenericUploadPictureByCarID" parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
