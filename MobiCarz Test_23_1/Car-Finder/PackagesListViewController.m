...
#warning  check when ur runing the App for method name
...
NSString *brandID = @"2";
    ...
    parameters = [NSDictionary dictionaryWithObjectsAndKeys:uid,@"UID",@"ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654",@"AuthenticationID",retrieveduuid,@"CustomerID",sessionID,@"SessionID",brandID,@"BrandID",nil];
    
    
    [self.Client setParameterEncoding:AFJSONParameterEncoding];
    [self.Client postPath:@"MobileService/GenericCarService.asmx/GenericAddPackageRequestMobile" parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
