...
#warning check running app for add new car
    self.Client = [[AFHTTPClient alloc] initWithBaseURL:[NSURL URLWithString:@"http://test1.unitedcarexchange.com/"]];
    
    NSString *brandID = @"1";
    NSDictionary * parameters = nil;
    
    parameters = [NSDictionary dictionaryWithObjectsAndKeys:make,@"make",model,@"model",price,@"price",year,@"year",mileage,@"mileage", uid,@"UID",userPackID,@"userPackID",packageID,@"packageID",autID,@"AuthenticationID",retrieveduuid,@"CustomerID",sessionID,@"SessionID",brandID,@"BrandID",nil];
  
    __weak AddNewCar *weakSelf=self;
    
   [self.Client setParameterEncoding:AFJSONParameterEncoding];
    [self.Client postPath:@"MobileService/GenericCarService.asmx/GenericAddCarDetails" parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
