...
//        rect = [[UIApplication sharedApplication] statusBarFrame];
        ...
        //load resources for iOS 7
        scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.scrollView1 attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.backgroundImageView attribute:NSLayoutAttributeTop multiplier:1 constant:self.navigationController.navigationBar.frame.size.height];
        //+rect.size.height
        [self.backgroundImageView addConstraint:scrollView1Constraint];
        
        scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.scrollView1 attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.backgroundImageView attribute:NSLayoutAttributeBottom multiplier:1 constant:-tabBarSize.height];
        [self.backgroundImageView addConstraint:scrollView1Constraint];
...
//    [self.label1 resizeToFit];
    ...
    if(!IsEmpty([self.carRecordFromFirstView phone]) && (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone))
    {
        ...
        callbutton.backgroundColor = [UIColor colorWithRed:226.0f/255.0f green:2.0f/255.0f blue:4.0f/255.0f alpha:1.0f];
    ...
    [self.viewGalleryBtn setTitleColor:[UIColor colorWithRed:105.0f/255.0f green:90.0f/255.0f blue:85.0f/255.0f alpha:1.0f] forState:UIControlStateNormal];
    //Button with 0 border so it's shape like image shape
   ...
   // [self.viewGalleryBtn setShowsTouchWhenHighlighted:YES];
    ...
    [self.featuresButton setTitleColor:[UIColor colorWithRed:105.0f/255.0f green:90.0f/255.0f blue:85.0f/255.0f alpha:1.0f] forState:UIControlStateNormal];
    //Button with 0 border so it's shape like image shape
    ...
    for (int i=0; i<[imageNames count]; i++) {
        ...
        if (![[self.imagesDictionary objectForKey:[imageNames objectAtIndex:i]] isEqualToString:@"Emp"]) {
           // completeimagename1=[[NSString alloc]initWithFormat:@"http://www.unitedcarexchange.com/%@%@",[self.imagesDictionary objectForKey:[imageDirs objectAtIndex:i]],[self.imagesDictionary objectForKey:[imageNames objectAtIndex:i]]];
            ...
            completeimagename1=[[NSString alloc]initWithFormat:@"http://images.unitedcarexchange.com/%@%@",[self.imagesDictionary objectForKey:[imageDirs objectAtIndex:i]],[self.imagesDictionary objectForKey:[imageNames objectAtIndex:i]]];
