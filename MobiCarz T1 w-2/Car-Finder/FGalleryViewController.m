...
- (void)loadView
{
    ...
    _toolbarTitle                       = [[UIToolbar alloc] initWithFrame:CGRectZero];
    ...
    _caption							= [[UILabel alloc] initWithFrame:CGRectZero];
     ...
     _toolbarTitle.barStyle              = UIBarStyleBlackTranslucent;
    
    _toolbar.barStyle					= UIBarStyleBlackTranslucent;
    if (_viewFromGalleryWithLoginSection ==YES || _selectedToGalleryViewFromLoginViews == YES)
    {
        _container.backgroundColor			= [UIColor colorWithRed:237/255.0 green:237/255.0 blue:237/255.0 alpha:1.0];
    }
    else
    {
    _container.backgroundColor			= [UIColor whiteColor];//colorWithRed:237/255.0 green:237/255.0 blue:237/255.0 alpha:1.0
    }
    ...
    // setup thumbs view
   // NSString *back3Str=[[NSBundle mainBundle] pathForResource:@"320-480" ofType:@"png"];
    //_thumbsView.backgroundColor					= [UIColor colorWithPatternImage:[UIImage imageWithContentsOfFile:back3Str]]; //[UIColor whiteColor];
    ...
    [_innerContainer addSubview:_toolbarTitle];
    
    
	[_innerContainer addSubview:_toolbar];
    
    
    [_toolbarTitle addSubview:_captionContainer];
    ...
    NSString *title=[NSString stringWithFormat:@"%d %@ %@",[self.carRecord year],[self.carRecord make],[self.carRecord model]];
    
    UILabel *titleLab = [[UILabel alloc] init];//WithFrame:CGRectMake(40, 4, 240, 36)];
    ...
    [_toolbarTitle addSubview:titleLab];
    //autolayout
    [titleLab setTranslatesAutoresizingMaskIntoConstraints:NO];
    NSLayoutConstraint *c1=[NSLayoutConstraint constraintWithItem:titleLab attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:_toolbarTitle attribute:NSLayoutAttributeCenterX multiplier:1 constant:0];
    [_toolbarTitle addConstraint:c1];
    
    c1=[NSLayoutConstraint constraintWithItem:titleLab attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:_toolbarTitle attribute:NSLayoutAttributeCenterY multiplier:1 constant:0];
    [_toolbarTitle addConstraint:c1];
...
- (void)viewDidUnload {
    ...
    [_toolbarTitle release], _toolbarTitle = nil;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    CGFloat y=[self AACStatusBarHeight]+self.navigationController.navigationBar.frame.size.height;
    
        _toolbarTitle.frame = CGRectMake(0, y, _scroller.frame.size.width, 40);
    
    //test whether image was uploaded just now
    if (self.justUploaded) {
        
        [self addCarToPhotoSource:self.carRecord];
...
- (void)resizeImageViewsWithRect:(CGRect)rect
{
	...
	for (i = 0; i < count; i++) {
        ...
        //photoView.frame = CGRectMake(dx, 0, rect.size.width, 100 );
		dx += rect.size.width;
	}
}
...
//
//    
//    UIBarButtonItem *lb= [[UIBarButtonItem alloc] init];
//    [lb setTitle:NSLocalizedString(@"Back", @"")];
//     [lb setCustomView:someButton];
//    self.navigationItem.leftBarButtonItem =lb;
//    lb=nil;
...
//            UIButton *someButton = [[UIButton alloc] initWithFrame:frameimg];
//            [someButton setTitle:NSLocalizedString(@"See all", @"") forState:UIControlStateNormal];
//            //someButton.titleLabel.text = NSLocalizedString(@"See all", @"");
//            someButton.titleLabel.textColor = [UIColor whiteColor];
//            
//            //[someButton setBackgroundImage:image3 forState:UIControlStateNormal];
//            [someButton addTarget:self action:@selector(handleSeeAllTouch:)
//                 forControlEvents:UIControlEventTouchUpInside];
//            [someButton setShowsTouchWhenHighlighted:YES];
//            
//            UIBarButtonItem *lb= [[UIBarButtonItem alloc] init];
//            //[lb setTitle:NSLocalizedString(@"See all", @"")];
//            
//            [lb setCustomView:someButton];
//            
//            [self.navigationItem setRightBarButtonItem:lb animated:YES];
//            lb=nil;
//            NSString *str = NSLocalizedString(@"See all", @"");
//            
//            
//            NSMutableAttributedString *now = [NSMutableAttributedString  attributedStringWithString:str];
//            
//            [now setTextColor:[UIColor whiteColor]];
            
            UIBarButtonItem *seeButton=[[UIBarButtonItem alloc] init];
            seeButton.target = self;
            seeButton.action = @selector(handleSeeAllTouch:);
            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:[UIColor whiteColor], UITextAttributeTextColor, nil];
            [seeButton setTitleTextAttributes:dic forState:UIControlStateNormal];
            [seeButton setTitle:NSLocalizedString(@"See all", @"")];
            seeButton.tintColor = [UIColor colorWithRed:216.0f/255.0f green:74.0f/255.0f blue:73.0f/255.0f alpha:1.0f];
...
//            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"See all", @"") style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] autorelease];
            [self.navigationItem setRightBarButtonItem:seeButton animated:YES];
        }
        else {
...
- (void)resizeThumbView
{
	...
	_thumbsView.frame = CGRectMake( 0, 0, _container.frame.size.width, _container.frame.size.height-barHeight );
     }
     else{
         
          //CGFloat width = [UIScreen mainScreen].bounds.size.width;
         barHeight = self.navigationController.navigationBar.frame.size.height+_toolbar.bounds.size.height;
         
         
             _thumbsView.frame = CGRectMake( 0, 100, _container.frame.size.width, _container.frame.size.height-barHeight);
...
-(void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    if(toInterfaceOrientation == UIInterfaceOrientationLandscapeLeft || toInterfaceOrientation == UIInterfaceOrientationLandscapeRight)
    {
       if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone) {
        ...
        _thumbsView.frame = CGRectMake( 0, 80, _container.frame.size.width, _container.frame.size.height-barHeight);
       }else{
           _thumbsView.frame = CGRectMake( 0, 100, _container.frame.size.width, _container.frame.size.height-barHeight);
       }
    }else{
...
- (void)buildThumbsViewPhotos
{
	...
	for (i = 0; i < count; i++) {
		...
		[thumbView setContentMode:UIViewContentModeScaleAspectFill]; //changed from aspectfill to aspectfit
...
- (void)addCarToPhotoSource:(CarRecord *)car
{
    SelectedCarDetails *selectedCarDetails=(SelectedCarDetails *)_photoSource;
    selectedCarDetails.carReceived=car;
    
    //then call retrieveUrlsAndImages on _photoSource and make sure the latest urls of images are retrieved
    
    [selectedCarDetails retrieveUrlsAndImages];
    //then reload gallery
    //[self reloadGallery];
}
...
@implementation UINavigationController (FGallery) // For iphone Rotation views
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
	...
	if([self.visibleViewController isKindOfClass:[FGalleryViewController class]])
	{
        ...
        if(toInterfaceOrientation == UIInterfaceOrientationLandscapeLeft || toInterfaceOrientation == UIInterfaceOrientationLandscapeRight)
        {
            ...
            galleryController.toolbarTitle.frame = CGRectMake(0, 44, height, 40);  //your landscape frame
                
            }
            else{
                CGFloat height = [UIScreen mainScreen].bounds.size.height;
                galleryController.toolbarTitle.frame = CGRectMake(0, 64, height, 40);
            }
        ...
        else
        {
            ...
            if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone) {
                ...
                CGFloat width = [UIScreen mainScreen].bounds.size.width;
            galleryController.toolbarTitle.frame = CGRectMake(0, 64, width, 40); // your portrait frame iphone
                
            }else{
               
                
                CGFloat width = [UIScreen mainScreen].bounds.size.width;
                galleryController.toolbarTitle.frame = CGRectMake(0, 64, width, 40); // your portrait frame ipad
            }
...
@implementation UITabBarController (FGallery)//// For ipad Rotation views
	...
	if([self.selectedViewController isKindOfClass:[UINavigationController class]])
	{
		...
		if([navController.visibleViewController isKindOfClass:[FGalleryViewController class]])
		{
            ...
            galleryController.toolbarTitle.frame = CGRectMake(0, 64, height, 40);
                ...
                if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone) {
                CGFloat width = [UIScreen mainScreen].bounds.size.width;
                 galleryController.toolbarTitle.frame = CGRectMake(0, 64, width, 40);
