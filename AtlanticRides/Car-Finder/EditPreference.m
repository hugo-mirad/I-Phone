...
- (void)viewDidLoad
{
    ...
    self.saveButton.backgroundColor = [UIColor colorWithRed:2.0f/255.0f green:152.0f/255.0f blue:215.0f/255.0f alpha:1.0f];
    ...
    // self.editPrefScrollView.contentSize = CGSizeMake(self.view.frame.size.width,450);
...
-(void)priceDoneButtonTapped
{
    ...
    //NSLog(@"self.view.frame.size.height--%f self.editPrefScrollView.contentSize.height--%f   self.editPrefScrollView.contentOffset.y--%f",self.view.frame.size.height,self.editPrefScrollView.contentSize.height,self.editPrefScrollView.contentOffset.y);
...

{
    ...
    if ([self.modelTextField isFirstResponder]) {
        if ([self.sortedModels count]==1) {
            ...
            NSLog(@"modelIdSelected=%@ modelNameSelected=%@",self.modelIdSelected,self.modelNameSelected);
        }
        else {
            self.modelIdSelected=@"0";
            self.modelNameSelected=@"All Models";
...

{
    ...
    if ([textField isEqual:self.makeTextField]) {
        ...
        // NSLog(@"Picker Frame = = = %@",NSStringFromCGRect(self.exteriorColorPicker.frame) );
