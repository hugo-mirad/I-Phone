    ...
    if([mArray count]>0)
    {
        if(self.loadingAtBottom)
        {
            ...
            if(testCounter>0)
            {
                ...
                /*
                 CGPoint collectionviewOffset2=[self.collectionView contentOffset];
                 CGPoint tempOffset=collectionviewOffset2;
                 tempOffset.y-=122*3;
                 collectionviewOffset2=tempOffset;
                 */
...
- (void)addCarsAtTopUsingResultArray:(NSArray *)mArray startingIndexPosition:(NSInteger)startingIndexPosition totalCarsToAdd:(NSInteger)carsToAdd
{
    ...
    NSLog(@"before adding at top to arrayOfAllSearchResultsCustomCellInfoObjects count=%d",self.arrayOfPreferenceTableCells.count);
    [self.arrayOfPreferenceTableCells insertObjects:mArray atIndexes:indexSetForTopRows];
    NSLog(@"after adding at top to arrayOfAllSearchResultsCustomCellInfoObjects count=%d",self.arrayOfPreferenceTableCells.count);
