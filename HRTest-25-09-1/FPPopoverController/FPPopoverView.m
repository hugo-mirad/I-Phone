...
- (void)drawRect:(CGRect)rect
{
    ...
    //content fill
    CGPathRef contentPath = [self newContentPathWithBorderWidth:4 arrowDirection:_arrowDirection];//2.0
    ...
    //external border
    CGPathRef externalBorderPath = [self newContentPathWithBorderWidth:2 arrowDirection:_arrowDirection];//1.0
    CGContextBeginPath(ctx);
...
-(void)setupViews
{
    ...
    else if(_arrowDirection == FPPopoverNoArrow)
    {
		...
		if (self.title==nil || self.title.length==0) {
			contentRect.origin = CGPointMake(10, 30);
			contentRect.size = CGSizeMake(self.bounds.size.width-10, self.bounds.size.height-20);
		}
    }
