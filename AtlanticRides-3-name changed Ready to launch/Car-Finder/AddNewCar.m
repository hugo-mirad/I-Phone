...
- (void)viewDidLoad
{
    ...
    NSLog(@"package details=%@",self.packageDetailsDict);
    
   NSLog(@"makesDictionary=%@",self.makesDictionary); 
    ...
    [titleLabel setText:@"Add A New Car"];
    ...
    makePickerLabel.textColor=[UIColor whiteColor];
    ...
    //self.makePickerTextField.placeholder = @"MakeMakeMakeMake";
   ...
   // self.mileageTextField.inputAccessoryView = toolbarForPicker;
    ...
    [self.addNewCarButton setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
    self.addNewCarButton.backgroundColor=[UIColor colorWithRed:213.0f/255.0f green:141.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
...
- (void)addNewCarButtonTapped
{
    ...
    NSLog(@"make--%@,model--%@,year--%@,price--%@,uid--%@,packageID--%@,autID--%@,retrieveduuid--%@,sessionID--%@",make,model,year, price,uid,packageID,autID,retrieveduuid,sessionID);
...
-(void)parserDidEndDocument:(NSXMLParser *)parser
{
    ...
    if ([self.currentElementChars isEqualToString:@"Success"]) {
        ...
        //update the '# of posted cars' field in package details-not necessary
