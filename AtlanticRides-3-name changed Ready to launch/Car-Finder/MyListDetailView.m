...
- (void)displayData
{
    ...
    UILabel *label;
    CGFloat lWidth;
    CGFloat y=10.0+self.tempImageView.frame.size.height+self.callView.frame.size.height;
    ...
    UIWebView *tempEmailButton=[[UIWebView alloc]initWithFrame:CGRectMake([CommonMethods findLabelWidth:@"Email:"]+4, self.tempImageView.frame.size.height+self.callView.frame.size.height, 200, 25)]; //when using as button give y as 8
    ...
    if(!IsEmpty([self.downloadedCarRecord phone]) && (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone))
    {
        ...
        callbutton.backgroundColor = [UIColor colorWithRed:2.0f/255.0f green:152.0f/255.0f blue:215.0f/255.0f alpha:1.0f];
...
-(void)errorDownloadingCarRecordNotifMethod:(NSNotification *)notif
{
    ...
    else
    {
        ...
        if ([error code]==kCFURLErrorNotConnectedToInternet) {
            ...
            self.serverErrorAlert.message=@"UCE Car Finder cannot retrieve data as it is not connected to the Internet.";
        }
        ...
        else
        {
            ...
            self.serverErrorAlert.message=@"UCE Car Finder cannot retrieve data because of server error.";
        }
