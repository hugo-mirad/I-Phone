...
- (void)viewDidLoad
{
    ...
    NSLayoutConstraint *carsListTableViewConstraint=[NSLayoutConstraint constraintWithItem:self.carsListTableView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeTop multiplier:1 constant:0];
...
- (void)viewDidUnload
{
    ...
    NSLog(@"arrayOfCarRecordsForThisPackage=%@",self.arrayOfCarRecordsForThisPackage);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        if ([[self.packageDetailsDict objectForKey:@"_CarsCount"] integerValue] < [[self.packageDetailsDict objectForKey:@"_MaxCars"] integerValue]) {
            ...
            addACar.backgroundColor = [UIColor colorWithRed:213.0f/255.0f green:141.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
            [addACar setTitle:@"ADD A CAR" forState:UIControlStateNormal];
            [addACar setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
