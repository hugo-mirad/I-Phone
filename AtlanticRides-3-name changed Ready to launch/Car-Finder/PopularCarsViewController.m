...
-(void)loadNextOrPreviousPage
{
    ...
    else if(self.currentPage>=5 && self.currentPage<=14 && (self.currentPage+1)<=self.totalPages && !self.operationStarted && (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad))
    {
        ...
        self.operationStarted=YES;
        //NSLog(@"after loading 6th page, current page no is %d",self.currentPage);
...
- (void)viewDidLoad
{
    ...
    navtitle.text=@"Atlantic Rides"; //UCE Car Finder
    ...
    if (self.zipStr==nil) {
        ...
        //NSLog(@"self.rightBarbutton setEnabled=%d",self.rightBarbutton.isEnabled);
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    if(self.currentPage==1)
    {
        ...
        if(!IsEmpty(self.arrayOfPopularCarsCellInfoObjects))
        {
            ...
            NSLog(@"count of arrayOfPopularCarsCellObjects in !IsEmpty=%d self.currentPage=%d",[self.arrayOfPopularCarsCellInfoObjects count],self.currentPage);
    ...
    if (self.arrayOfPopularCarsCellInfoObjects.count>0)
    {
        ...
        if([[pcCellInfo car] carid])
        {
            ...
            else
            {
                [cell.spinner startAnimating];
                cell.imageView.image = [[UIImage alloc] initWithCIImage:nil];
                //           NSLog(@"image should be downloaded for %@ - %@ - %d image=%@",pcCellInfo.car.make,pcCellInfo.car.model,pcCellInfo.car.price,pcCellInfo.car.thumbnailUIImage);
...
- (void)addCarsUsingResultArray:(NSArray *)mArray startingIndexPosition:(NSInteger)startingIndexPosition totalCarsToAdd:(NSInteger)carsToAdd
{
    ...
    //NSLog(@"before adding cars array=%@ count is %d",self.arrayOfPopularCarsCellInfoObjects,self.arrayOfPopularCarsCellInfoObjects.count);
    [self.arrayOfPopularCarsCellInfoObjects addObjectsFromArray:mArray]; /////
    //NSLog(@"after adding cars array=%@ count is %d",self.arrayOfPopularCarsCellInfoObjects,self.arrayOfPopularCarsCellInfoObjects.count);
    //
...
- (void)deleteCarsFromBottomWithResultArray:(NSArray *)mArray startingIndexPosition:(NSInteger)lastPageStartingIndex totalCarsToDelete:(NSInteger)carsToDelete
{
    ...
    NSLog(@"before deleting from bottom arrayOfPopularCarsCellInfoObjects count=%d",self.arrayOfPopularCarsCellInfoObjects.count);
    
    [self.arrayOfPopularCarsCellInfoObjects removeObjectsAtIndexes:indexSet4];
    NSLog(@"after deleting from bottom arrayOfPopularCarsCellInfoObjects count=%d",self.arrayOfPopularCarsCellInfoObjects.count);
    ...
    if([mArray count]>0)
    {
        if(self.loadingAtBottom)
        {
            ...
            else
            {
                ...
                //NSLog(@"items in array=%d cells in collection view=%d new items to add=%d new cells to add=%d",self.arrayOfPopularCarsCellInfoObjects.count,[self.collectionView numberOfItemsInSection:0],mArray.count,[cellIndicesToAdd count]);
                [self.arrayOfPopularCarsCellInfoObjects addObjectsFromArray:mArray];
        ...
        else if(self.loadingAtTop)
        {
            ...
            if(testCounter>0)
            {
                ...
                __weak PopularCarsViewController *weakSelf=self;
                //NSLog(@"lastPageStartingIndex=%d numberOfCarsToDelete=%d",lastPageStartingIndex,numberOfCarsToDelete);
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSLog(@"we are at iPath.item=%d",iPath.item);
    ...
    if (iPath.item<=topReachLimit && !self.operationStarted) { //currentOffset<=3*122
        ...
        NSLog(@"we are at the end. iPath.item=%d",iPath.item);
        if(self.loadRowsAtEndCounterMain==1)
        {
            if(self.currentPage+1<=self.totalPages)
            {
