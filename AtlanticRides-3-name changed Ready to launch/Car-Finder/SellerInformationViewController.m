...
- (void)viewDidLoad
{
    ...
    self.topNoteLabel.textColor=[UIColor whiteColor];
    ...
    sellerInfoPhoneNumLabel.textColor=[UIColor whiteColor];
    ...
    sellerInfoCityLabel.textColor=[UIColor whiteColor];
    ...
    sellerInfoZipCodeLabel.textColor=[UIColor whiteColor];
    ...
    sellerInfoStateLabel.textColor=[UIColor whiteColor];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if ([pickerView isEqual:self.statePicker]) {
        ...
        NSLog(@"self.stateIdSelected=%@-%@",self.stateIdSelected,self.stateSelected);
...
- (BOOL)userMadeChanges
{
    ...
    //BOOL emailNotChanged=[self.emailIdTextField.text isEqualToString:[self.carReceived email]];
     ...
     NSLog(@"self.stateSelected=%@---[self.carReceived state]--%@",self.stateSelected,[self.carReceived state]);
...
- (void)rightBarButtonTapped:(id) sender
{
    ...
    else
    {
        ...
        if ([self userMadeChanges]) {
            NSLog(@"self.stateSelected--%@  self.stateTF--%@",self.stateSelected,self.stateTextField.text);
...
////    //get statename from stateid
    ...
    NSLog(@"stateCode--%@  self.stateIdSelected--%@",stateCode,self.stateIdSelected);
...
//    NSArray *values=[NSArray arrayWithObjects:sellerId,sellerName,city,stateCode,zip,phone,email,carId,uID,@"ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654",retrieveduuid,sessionID1,nil];
//    
//    NSArray *keys=[NSArray arrayWithObjects:@"sellerID",@"sellerName",@"city",@"state",@"zip",@"phone",@"email",@"carId",@"UID",@"AuthenticationID",@"CustomerID",@"SessionID",nil];
//    
    ...
    [Client postPath:@"MobileService/CarService.asmx/UpdateSellerInformation" parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject)
     ...
     self.rightBarButton.enabled=YES;
         ...
         NSLog(@"operation hasAcceptableStatusCode: %@", operation.response );
...
- (void)webServiceCallToSaveDataSucceededWithResponse:(NSString *)aDict
{
    ...
    NSLog(@"self.stateSelected-- %@",self.stateTextField.text);
...
NSLog(@"tf--- %@",self.stateTextField.text);
...
-(void)retrieveLoggedUserPreviousData
{
    ...
    if (self.carReceived) {
       ...
       //NSString *stateStr = @"AK";
        NSString *stateStr = [self.carReceived state];
        
        NSLog(@"State %@",stateStr);
        ...
        NSLog(@"State %@--indexOfState--%d",stateStr,indexOfState);
