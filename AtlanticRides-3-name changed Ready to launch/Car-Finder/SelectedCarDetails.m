...
- (void)viewDidLoad
{
    ...
    UILabel *navtitle=[[UILabel alloc]initWithFrame:CGRectMake(self.view.frame.size.width/2, 0, 200, 45)];
    //navtitle.text=@"Login"; //
    ...
    [navtitle setAdjustsFontSizeToFitWidth:YES];
    [navtitle setLineBreakMode:NSLineBreakByClipping];
...
-(void)retrieveUrlsAndImages
{
    ...
    NSLog(@"car in retrieveUrlsAndImages is %@",self.carReceived);
    //calling service
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSLog(@"findCarIDResult=%@",findCarIDResult);
...
-(void)getArrayOfCarPicUrls
{
    ...
    NSLog(@"Car record is %@ All the pic urls for this car are %@",self.carReceived,self.arrayOfCarPicUrls);
    //if count>0, we proceed with gallery, otherwise we display empty gallery(if possible) or show another screen where user can take photo
    if (self.networkGallery!=nil) {
        //self.networkGallery=nil;

        [self prepareNetworkGallery];
        NSLog(@"navigation stack is %@",self.navigationController.viewControllers);
        //self.networkGallery.beginsInThumbnailView=NO;
        [self.networkGallery reloadGallery];
        //[self.navigationController pushViewController:self.networkGallery animated:NO];
    }
...
{
    NSLog(@"doneButtonTapped: navigation stack is %@",self.navigationController.viewControllers);
    
    NSString *eColor= [self.carReceived exteriorColor];
    NSLog(@"eColor=%@",eColor);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        ...
        case 6:
            ...
            //NSLog(@"adStatus is %@xyz",adStatus);
            if (adStatus!=nil) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        ...
        case 2:
            ...
            NSLog(@"self.carReceived--%@",self.carReceived);
            break;
...
-(void)featureDetailsImplementation
{
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if(error2==nil)
        {
            ...
            //NSLog(@"weakSelf.featuresArray=%@",weakSelf.featuresArray);
...
- (void)callWebServiceToSaveData:(NSInteger)buttonIndex
{
    ...
    NSLog(@"updateCarDetailsServiceStr=%@",updateCarDetailsServiceStr);
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        //NSLog(@"service call succeeded. Operations reponse string is %@. responseObject class is %@, responseObject=%@",[operation responseString], NSStringFromClass([responseObject class]), responseDict);
        if(error2==nil)
        {
