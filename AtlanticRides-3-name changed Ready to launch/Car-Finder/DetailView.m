...

{
    ...
    // call service
    //http://www.unitedcarexchange.com/MobileService/ServiceMobile.svc/SaveCallRequestMobile/0/381/7244650258/ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654/12345
    
    
    NSString *retrieveduuid = [SSKeychain passwordForService:UUID_USER_DEFAULTS_KEY account:@"user"];
    
    
    callServiceStr=[NSString stringWithFormat:@"http://www.unitedcarexchange.com/MobileService/ServiceMobile.svc/SaveCallRequestMobile/0/%d/%@/ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654/%@",[self.carRecordFromFirstView carid],[self.carRecordFromFirstView phone],retrieveduuid];
    
    
    // callServiceStr=[NSString stringWithFormat:@"http://unitedcarexchange.com/MobileService/Service.svc/SaveCallRequestMobile/0/%d/%@/",[self.carRecordFromFirstView carid],[self.carRecordFromFirstView phone]];
    
    //        http://unitedcarexchange.com/MobileService/Service.svc/SaveCallRequestMobile/0/381/7244650258/
...
-(void)checkForFeatures
{
    ...
    // GetCarFeatures (detail view)
    //http://www.unitedcarexchange.com/MobileService/ServiceMobile.svc/GetCarFeatures/1902/ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654/12345/
    
    
    NSString *retrieveduuid = [SSKeychain passwordForService:UUID_USER_DEFAULTS_KEY account:@"user"];
    
    NSString *featuresServiceStr=[NSString stringWithFormat:@"http://www.unitedcarexchange.com/MobileService/ServiceMobile.svc/GetCarFeatures/%d/ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654/%@/",[self.carRecordFromFirstView carid],retrieveduuid];
    
    
    // NSString *featuresServiceStr=[NSString stringWithFormat:@"http://unitedcarexchange.com/MobileService/Service.svc/GetCarFeatures?sCarId=%d",[self.carRecordFromFirstView carid]];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if(error2==nil)
        {
            ...
            else
            {
                ...
                //NSLog(@"features not found for this");
...
- (void)viewDidLoad
{
    ...
    [defaults setValue:navTitle forKey:@"navTitleDetail"];
    ...
    //NSLog(@"car id from previous scene is %d",[self.carRecordFromFirstView carid]);
    ...
    NSLayoutConstraint *scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.scrollView1 attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationEqual toItem:self.backgroundImageView attribute:NSLayoutAttributeLeading multiplier:1 constant:0];
    [self.backgroundImageView addConstraint:scrollView1Constraint];
    
    scrollView1Constraint=[NSLayoutConstraint constraintWithItem:self.scrollView1 attribute:NSLayoutAttributeTrailing relatedBy:NSLayoutRelationEqual toItem:self.backgroundImageView attribute:NSLayoutAttributeTrailing multiplier:1 constant:0];
    [self.backgroundImageView addConstraint:scrollView1Constraint];
    ...
    self.tempImageView.frame=CGRectMake(self.view.frame.size.width/2-270/2,10,270,164);
    ...
    //
    //  self.scrollView1=[[UIScrollView alloc] initWithFrame:CGRectMake(0,self.tempImageView.frame.size.height+self.myListView.frame.size.height,self.view.frame.size.width,self.view.frame.size.height-self.tempImageView.frame.size.height-self.myListView.frame.size.height-self.tabBarController.tabBar.frame.size.height-self.navigationController.navigationBar.frame.size.height)]; //150 49 for tab bar
    ...
    UILabel *label;
    CGFloat lWidth;
    CGFloat y=10.0+self.tempImageView.frame.size.height+self.myListView.frame.size.height;
    //
    ...
    //
    self.emailButton=[[UIWebView alloc]initWithFrame:CGRectMake([CommonMethods findLabelWidth:@"Email:"]+4, self.tempImageView.frame.size.height+self.myListView.frame.size.height, 200, 25)]; //when using as button give y as 8
    ...
    NSString *testString = @"<a href = \"obj://emailButtonTapped\"><font color='orange'>Send email to seller</color></a>";
    ...
    self.descLbl.textColor = [UIColor whiteColor];
    // NSLog(@"label--%@",descLbl.text);
    [self.scrollView1 addSubview:self.descLbl];
    // NSLog(@"fieldVal--%@",fieldVal);
    ...
    if(!IsEmpty([self.carRecordFromFirstView phone]) && (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone))
    {
        ...
        callbutton.backgroundColor = [UIColor colorWithRed:2.0f/255.0f green:152.0f/255.0f blue:215.0f/255.0f alpha:1.0f];//[UIColor colorWithRed:226.0f/255.0f green:2.0f/255.0f blue:4.0f/255.0f alpha:1.0f];
        [callbutton setTitle:@"Call" forState:UIControlStateNormal];
        [callbutton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        //Button with 0 border so it's shape like image shape
        callbutton.layer.shadowRadius = 2.0f;
    ...
    self.viewGalleryBtn.backgroundColor = [UIColor colorWithRed:213.0f/255.0f green:141.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
    [self.viewGalleryBtn setTitle:@"Gallery" forState:UIControlStateNormal];
    [self.viewGalleryBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];//[UIColor colorWithRed:105.0f/255.0f green:90.0f/255.0f blue:85.0f/255.0f alpha:1.0f]
    //Button with 0 border so it's shape like image shape
    self.viewGalleryBtn.layer.shadowRadius = 2.0f;
   ...
   // [self.viewGalleryBtn setShowsTouchWhenHighlighted:YES];
    ...
    self.featuresButton.backgroundColor = [UIColor colorWithRed:213.0f/255.0f green:141.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
    ...
    //Button with 0 border so it's shape like image shape
    self.featuresButton.layer.shadowRadius = 2.0f;
    ...
    self.myListBtn.frame=CGRectMake(250,8, mylistImageSize.width, mylistImageSize.height);
                                          ...
                                          duration:(NSTimeInterval)duration
{
    ...
    {
        self.label1.frame = CGRectMake(10.0f,594,self.view.frame.size.width-10,descLabelHeight);  //your landscape frame
        
    }
    else
    {
        self.label1.frame = CGRectMake(10.0f, 594, self.view.frame.size.width-10, descLabelHeight); // your portrait frame
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        weakSelf.tempImageView.frame = CGRectMake(self.view.bounds.size.width/2-270/2,10,270,164);
      //  weakSelf.descLbl.preferredMaxLayoutWidth = weakSelf.view.bounds.size.width;
    });
...
-(void)getArrayOfCarPicUrls
{
    ...
    //NSLog(@"imagesDictionary dictionary is %@",self.imagesDictionary);
...
-(void)retrieveUrlsAndImages
{
    ...
    // FindCarID (gallery)
    //http://www.unitedcarexchange.com/MobileService/ServiceMobile.svc/FindCarID/381/ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654/12345
    
    NSString *retrieveduuid = [SSKeychain passwordForService:UUID_USER_DEFAULTS_KEY account:@"user"];
    
    NSString *webServiceUrl=[NSString stringWithFormat:@"http://www.unitedcarexchange.com/MobileService/ServiceMobile.svc/FindCarID/%d/ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654/%@",[self.carRecordFromFirstView carid],retrieveduuid];
    
    
    
    //calling service
    
    //NSString *webServiceUrl=[NSString stringWithFormat:@"http://unitedcarexchange.com/MobileService/Service.svc/FindCarID/%d/",[self.carRecordFromFirstView carid]];
