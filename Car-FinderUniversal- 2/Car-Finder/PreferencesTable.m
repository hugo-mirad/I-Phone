...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[prefDict objectForKey:@"name"] isEqualToString:@"Preference0"])
    {
        ...
        addPreferenceBtn.backgroundColor = [UIColor colorWithRed:241.0f/255.0f green:240.0f/255.0f blue:240.0f/255.0f alpha:1.0f];
        [addPreferenceBtn setTitle:@"ADD PREFERENCE" forState:UIControlStateNormal];
        [addPreferenceBtn setTitleColor:[UIColor colorWithRed:105.0f/255.0f green:90.0f/255.0f blue:85.0f/255.0f alpha:1.0f] forState:UIControlStateNormal];
        //Button with 0 border so it's shape like image shape
        addPreferenceBtn.layer.shadowRadius = 2.0f;
    ...
    //
        ...
        NSLayoutConstraint *editPreferenceBtnConstraint=[NSLayoutConstraint constraintWithItem:editPreferenceBtn attribute:NSLayoutAttributeTrailing relatedBy:NSLayoutRelationEqual toItem:cell.contentView attribute:NSLayoutAttributeTrailing multiplier:1 constant:-20];
        ...
        editPreferenceBtnConstraint=[NSLayoutConstraint constraintWithItem:editPreferenceBtn attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1 constant:32];
        [cell.contentView addConstraint:editPreferenceBtnConstraint];
        
        editPreferenceBtnConstraint=[NSLayoutConstraint constraintWithItem:editPreferenceBtn attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1 constant:32];
        [cell.contentView addConstraint:editPreferenceBtnConstraint];
        ...
        if (indexPath.row%2==1) {
            cell.makeModelLabel.textColor=[UIColor whiteColor];
            cell.yearMileageLabel.textColor=[UIColor whiteColor];
            cell.priceLabel.textColor=[UIColor whiteColor];
            cell.totalCarsFoundLabel.textColor=[UIColor whiteColor];
            cell.backgroundColor = [UIColor clearColor];
        }
        else
        {
...
-(void)startPreferenceDownloadOp:(NSMutableDictionary *)prefDict
{
    ...
    NSString *callServiceStr=[NSString stringWithFormat:@"http://www.unitedcarexchange.com/MobileService/ServiceMobile.svc/GetCarsFilterMobile/%@/%@/%@/%@/%@/asc/price/%d/%d/%@/ds3lkDFD1F5fFGnf2daFs45REsd6re54yb0sc654/%@",makeIdReceived,modelIdReceived,mileageReceived,yearReceived,priceReceived,pageSizeReceived,pageNoReceived,zipReceived,retrieveduuid];
    
    
     //  NSString *callServiceStr=[NSString stringWithFormat:@"http://unitedcarexchange.com/MobileService/Service.svc/GetCarsFilterMobile/%@/%@/%@/%@/%@/asc/price/%d/%d/%@",makeIdReceived,modelIdReceived,mileageReceived,yearReceived,priceReceived,pageSizeReceived,pageNoReceived,zipReceived];
